...
app.on("ready", async () => {
  let primary = electron.screen.getPrimaryDisplay().bounds;
  let displays = electron.screen.getAllDisplays();
  let externalDisplay = displays.find((display) => {
    return display.bounds.x != primary.x || display.bounds.y != primary.y;
  });

  if (externalDisplay) {
    createWindow(externalDisplay.bounds.x, externalDisplay.bounds.y);
  }
});
